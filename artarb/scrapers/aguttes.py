"""Scraper for Aguttes — French auction house (Paris / Lyon).

Focus: Alechinsky, Bram van Velde, French modern prints, CoBrA.
"""
...
FIELD_SELECTORS = {
    ...
    "hammer":   [".hammer", ".adjudication", "[class*='hammer']", ".result-price"],
    "date":     ["time[datetime]", ".date-vente"],
}
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        ...
        items_found = len(raw)
        log.info("aguttes page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("aguttes item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("aguttes page=%d %s", page, exc)
    finally:
