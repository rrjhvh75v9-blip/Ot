"""Scraper for Ketterer Kunst — German auction house (Munich / Hamburg).

Focus: German Expressionism, Picasso, post-war international,
important modern prints.
...
COUNTRY_SALE = "DE"
CURRENCY = "EUR"
BASE_URL = "https://www.kettererkunst.de"
RESULTS_PATH = "/ergebnisse"
RATE_LIMIT_SEC = 3
REQUEST_TIMEOUT = 30

ITEM_SELECTORS = [
    "[data-lot-id]", "article.lot", ".lot-ergebnis",
    "[class*='lot-item']", "[class*='ergebnis']",
    "li.lot", "article", "tr.lot",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/detail']", "a"],
    "title":    [".lot-title", ".werktitel", ".titel", "h2", "h3"],
    "artist":   [".artist", ".kuenstler", "[class*='artist']"],
    "technique":[".technique", ".technik", ".material"],
    "dims":     [".dimensions", ".masse", "[class*='dimension']"],
    "estimate": [".estimate", ".schaetzpreis", "[class*='estimate']"],
    "hammer":   [".hammer", ".zuschlag", ".ergebnis-preis", "[class*='hammer']"],
    "date":     ["time[datetime]", ".auktions-datum", "[class*='date']"],
}
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    items_found = 0
    try:
        html = try_playwright_get(url, wait_selector="[data-lot-id], article.lot, .lot-ergebnis")
        if html is None:
        ...
        items_found = len(raw)
        log.info("ketterer page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("ketterer item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("ketterer page=%d %s", page, exc)
    finally:
...
def _lot_id(url: Optional[str]) -> Optional[str]:
    if not url:
        return None
    m = re.search(r"/lot[s]?/([^/?#]+)|/detail/([^/?#]+)|/(\d{4,})", url)
