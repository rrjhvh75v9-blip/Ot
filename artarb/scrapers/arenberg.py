"""Scraper for Arenberg Auctions — Belgian auction house (Brussels).

Focus: Belgian modern masters, Ensor, Spilliaert, Belgian Symbolism.
"""
...
COUNTRY_SALE = "BE"
CURRENCY = "EUR"
BASE_URL = "https://www.arenberg-auctions.com"
LISTINGS_PATH = "/nl/veilingen"
RATE_LIMIT_SEC = 3
REQUEST_TIMEOUT = 25

ITEM_SELECTORS = ["article.lot", ".lot-item", "[data-lot]", "div.kavel", "article", "li"]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/kavel']", "a"],
    "title":    [".lot-title", ".lot-name", "h2", "h3"],
    "artist":   [".artist", ".kunstenaar", ".auteur", "[data-artist]"],
    "technique":[".technique", ".techniek"],
    "dims":     [".dimensions", ".afmetingen"],
    "estimate": [".estimate", ".schatting", "[class*='estimate']"],
    "hammer":   [".hammer", ".toewijzing", ".result", "[class*='result']"],
    "date":     ["time[datetime]", ".date", ".datum"],
}
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        ...
        items_found = len(raw)
        log.info("arenberg page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("arenberg item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("arenberg page=%d %s", page, exc)
    finally:
