"""Scraper for Galerie Bassenge — German auction house (Berlin).

Focus: Kollwitz, Kubin, German Expressionist prints, manuscripts.
Targets the results/Auktionen pages for sold price data.
"""
...
COUNTRY_SALE = "DE"
CURRENCY = "EUR"
BASE_URL = "https://www.bassenge.com"
RESULTS_PATH = "/auktionen"
RATE_LIMIT_SEC = 3
REQUEST_TIMEOUT = 25

ITEM_SELECTORS = [
    "tr.lot-row", "div.lot", "article.lot",
    "[class*='lot-item']", "[data-lot]", "li.lot", "article",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/ergebnis']", "a"],
    "title":    [".lot-title", ".werk-titel", "h2", "h3", "td.title"],
    "artist":   [".lot-artist", ".kuenstler", ".artist", "td.artist"],
    "technique":[".technique", ".technik", "td.technique"],
    "dims":     [".dimensions", ".masse", "td.dimensions"],
    "estimate": [".estimate", ".schaetzpreis", "td.estimate", "[class*='estimate']"],
    "hammer":   [".hammer", ".zuschlag", "td.hammer", "[class*='hammer']", ".result"],
    "date":     ["time[datetime]", ".auction-date", ".datum"],
}


def scrape(max_pages: Optional[int] = None) -> None:
    http = build_http_session({"Referer": BASE_URL, "Accept-Language": "de-DE,de;q=0.9"})
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    url = f"{BASE_URL}{RESULTS_PATH}?seite={page}"
    ...
    try:
        ...
        items_found = len(raw)
        log.info("bassenge page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("bassenge item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("bassenge page=%d %s", page, exc)
    finally:
...
def _parse_item(tag: Tag) -> Optional[dict]:
    ...
    return {
        ...
        "source_lot_id": tag.get("data-lot-id") or tag.get("data-lot") or _lot_id(href),
...
def _lot_id(url: Optional[str]) -> Optional[str]:
    if not url:
        return None
    m = re.search(r"/lot[s]?/([^/?#]+)|/ergebnis/([^/?#]+)|/(\d{4,})", url)
