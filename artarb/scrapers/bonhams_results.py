"""Scraper for Bonhams — International auction house.

Focus: post-war international, Prints & Multiples, Design,
Asian art. Currency detection handles GBP vs EUR lots.
...
ITEM_SELECTORS = [
    ...
    "[class*='LotItem']", "[class*='lot-card']",
    "[class*='result-item']", "article", "li",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/auction']", "a"],
    "title":    [".lot-title", ".LotItem__title", ".artwork-title", "h2", "h3"],
    "artist":   [".lot-artist", ".LotItem__artist", ".artist-name", "[class*='artist']"],
    "technique":[".technique", ".medium", ".LotItem__description"],
    "dims":     [".dimensions", ".LotItem__dimensions", "[class*='dimension']"],
    "estimate": [".estimate", ".LotItem__estimate", "[class*='estimate']"],
    "hammer":   [".hammer-price", ".LotItem__price", ".sold-price", "[class*='hammer']"],
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        html = try_playwright_get(
            url,
            wait_selector="[data-lot-id], article.lot, [class*='LotItem'], [class*='lot-card']",
            timeout_ms=40_000,
        )
        if html is None:
        ...
        items_found = len(raw)
        log.info("bonhams page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("bonhams item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("bonhams page=%d %s", page, exc)
    finally:
...
def _lot_id(url: Optional[str]) -> Optional[str]:
    if not url:
        return None
    m = re.search(r"/lot[s]?/([^/?#]+)|/auction/[^/?#]+/lot/([^/?#]+)|/(\d{4,})", url)
