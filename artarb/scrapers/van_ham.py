"""Scraper for Van Ham — German auction house (Cologne).

Focus: post-war German art, Dieter Roth, CoBrA.
Targets results pages for hammer price data.
"""
...
COUNTRY_SALE = "DE"
CURRENCY = "EUR"
BASE_URL = "https://auction.van-ham.com"
RESULTS_PATH = "/en/results"
RATE_LIMIT_SEC = 3
REQUEST_TIMEOUT = 25

ITEM_SELECTORS = [
    "[data-lot-id]", "article.lot", "div.lot-result",
    "[class*='lot-item']", "[class*='result-item']", "article", "li.lot",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/result']", "a"],
    "title":    [".lot-title", ".artwork-title", "h2", "h3"],
    "artist":   [".lot-artist", ".artist-name", "[class*='artist']"],
    "technique":[".technique", ".medium", "[class*='technique']"],
    "dims":     [".dimensions", "[class*='dimension']"],
    "estimate": [".estimate", "[class*='estimate']", ".price-estimate"],
    "hammer":   [".hammer-price", ".result-price", "[class*='hammer']", ".lot-price"],
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        ...
        items_found = len(raw)
        log.info("van_ham page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("van_ham item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("van_ham page=%d %s", page, exc)
    finally:
