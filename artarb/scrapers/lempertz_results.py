"""Scraper for Lempertz — German auction house (Cologne / Berlin).

Focus: sold price data for Alechinsky, Karel Appel, Bram van Velde,
CoBrA, post-war German and Belgian art.
"""
...
COUNTRY_SALE = "DE"
CURRENCY = "EUR"
BASE_URL = "https://www.lempertz.com"
RESULTS_PATH = "/en/catalogues"
RATE_LIMIT_SEC = 3
REQUEST_TIMEOUT = 25

ITEM_SELECTORS = [
    "article.lot", "tr.lot", ".lot-result",
    "[data-lot-id]", "[class*='lot-item']", "article", "li.lot",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/katalog']", "a"],
    "title":    [".lot-title", ".werk-titel", "h2", "h3", "td.title"],
    "artist":   [".lot-artist", ".artist", ".kuenstler", "td.artist"],
    "technique":[".technique", ".technik", "td.technique"],
    "dims":     [".dimensions", ".masse", "td.dimensions"],
    "estimate": [".estimate", ".schaetzung", "td.estimate"],
    "hammer":   [".hammer", ".zuschlag", "td.hammer", ".ergebnis", "[class*='result']"],
    "date":     ["time[datetime]", ".auction-date", "td.date"],
}


def scrape(max_pages: Optional[int] = None) -> None:
    http = build_http_session({"Referer": BASE_URL, "Accept-Language": "en-GB,en;q=0.9"})
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        ...
        items_found = len(raw)
        log.info("lempertz page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("lempertz item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("lempertz page=%d %s", page, exc)
    finally:
