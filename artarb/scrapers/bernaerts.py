"""Scraper for Bernaerts — Belgian auction house (Antwerp).

Focus: Belgian modern art, CoBrA, Spilliaert, Ensor.
"""
...
COUNTRY_SALE = "BE"
CURRENCY = "EUR"
BASE_URL = "https://www.bernaerts.be"
LISTINGS_PATH = "/nl/veilingen"
RATE_LIMIT_SEC = 3
REQUEST_TIMEOUT = 25

ITEM_SELECTORS = ["article.lot", ".lot-item", "[data-lot]", "div.kavel", "article", "li.lot"]
FIELD_SELECTORS = {
    "link":     ["a[href*='/kavel']", "a[href*='/lot']", "a"],
    "title":    [".lot-title", ".lot-naam", "h2", "h3"],
    "artist":   [".lot-artist", ".kunstenaar", ".auteur", "[data-artist]"],
    "technique":["technique", ".techniek", ".materiaal"],
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        ...
        items_found = len(raw)
        log.info("bernaerts page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("bernaerts item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
        log.error("bernaerts page=%d %s", page, error_msg)
    except Exception as exc:
        error_msg = str(exc)
        log.error("bernaerts page=%d %s", page, exc)
    finally:
