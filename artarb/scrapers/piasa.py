"""Scraper for PIASA — French auction house (Paris).

Focus: Czech avant-garde prices, Toyen, Kupka, French modern art.
Targets the /resultats page for sold price data.
"""
...
ITEM_SELECTORS = [
    ...
    "[class*='lot-card']", ".result-lot", "article", "li.lot",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/oeuvre']", "a"],
    "title":    [".lot-title", ".artwork-title", "h2", "h3", ".titre"],
    "artist":   [".lot-artist", ".artiste", ".artist", "[class*='artist']"],
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    ...
    try:
        ...
        items_found = len(raw)
        log.info("piasa page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("piasa item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("piasa page=%d %s", page, exc)
    finally:
