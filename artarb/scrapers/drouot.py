"""Scraper for Hôtel Drouot — French auction aggregator (Paris).

Focus: French market breadth — covers hundreds of maisons de vente,
excellent for price discovery on mid-market French and Belgian works.
Requires Playwright (JavaScript-rendered lot grid).
"""
...
ITEM_SELECTORS = [
    "[data-lot-id]", "article.lot", ".lot-card",
    "[class*='LotCard']", "[class*='lot-item']",
    "[class*='result-item']", "article", "li",
]
FIELD_SELECTORS = {
    "link":     ["a[href*='/lot']", "a[href*='/vente']", "a"],
    "title":    [".lot-title", ".LotCard__title", ".titre", "h2", "h3"],
    "artist":   [".artist", ".LotCard__artist", ".artiste", "[class*='artist']"],
    "technique":[".technique", ".medium", ".description"],
    "dims":     [".dimensions", ".mesures"],
    "estimate": [".estimate", ".LotCard__estimate", ".estimation", "[class*='estimate']"],
    "hammer":   [".hammer", ".LotCard__result", ".adjudication", "[class*='hammer']"],
    "date":     ["time[datetime]", ".date-vente", "[class*='date']"],
}
...
def _scrape_page(db: Session, http: requests.Session, page: int) -> tuple[int, Optional[str]]:
    url = f"{BASE_URL}{LOTS_PATH}?page={page}"
    ...
    try:
        html = try_playwright_get(
            url,
            wait_selector="[data-lot-id], article.lot, .lot-card, [class*='LotCard']",
            timeout_ms=40_000,
        )
        if html is None:
        ...
        items_found = len(raw)
        log.info("drouot page=%d items=%d", page, items_found)
        for item in raw:
            try:
                upsert_item(db, PLATFORM, item)
            except Exception as exc:
                log.error("drouot item error: %s", exc)
    except requests.HTTPError as exc:
        error_msg = f"HTTP {exc.response.status_code}"
    except Exception as exc:
        error_msg = str(exc)
        log.error("drouot page=%d %s", page, exc)
    finally:
...
def _lot_id(url: Optional[str]) -> Optional[str]:
    if not url:
        return None
    m = re.search(r"/lot[s]?/([^/?#]+)|/vente/[^/?#]+/(\d+)|/(\d{4,})", url)
